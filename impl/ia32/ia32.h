...
// x64 Windows _MDL (Vergilius): Size/MdlFlags are SHORT; Process @ +0x10.
// PFN array begins immediately after this 0x30-byte header.
struct mdl_t {
    struct mdl_t* m_next;              // +0x00
    std::uint16_t m_size;              // +0x08  (CSHORT)
    std::uint16_t m_mdl_flags;         // +0x0A  (CSHORT)
    std::uint32_t m_pad;               // +0x0C  alignment to Process
    struct eprocess_t* m_process;      // +0x10
    void* m_mapped_system_va;          // +0x18
    void* m_start_va;                  // +0x20
    std::uint32_t m_byte_count;        // +0x28
    std::uint32_t m_byte_offset;       // +0x2C
};
static_assert( sizeof( mdl_t ) == 0x30, "MDL header must be 48 bytes on x64" );
